...
import requests, telebot, jwt, os, sys, pickle, json, binascii, time, urllib3, xKEys, base64, datetime, re, socket, threading
import asyncio
...
# للحصول على البورت من متغيرات البيئة
...
BOT_TOKEN = "8764257074:AAEs9T_1N2FR-vZA1QC8gGbKpdH14nhIyzg"
...
def telegram_spam(message):
    try:
        parts = message.text.split()

        if len(parts) < 2:
            bot.reply_to(message, "❌ الاستعمال:\n/spam user_id duration")
            return
        ...
        result = api.process_spam_command(user_id, duration)
...
def telegram_stop(message):
    try:
        ...
        result = api.process_stop_command(user_id)
...
class FF_CLient():
    ...
    def Guest_GeneRaTe(self , uid , password):
        ...
        try:
            ...
            time.sleep(0.2)
            print(' - Starting aMiNe KiNg Freind BoT !')
            print(f' - Uid : {uid}\n - Password : {password}')
